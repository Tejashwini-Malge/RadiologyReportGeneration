"""
02_extract_features.py  (v3 -- adds image_id + completeness flag)
...
Output per split:  <features_dir>/<split>.h5
...
Usage:
    python 02_extract_features.py --split valid --limit 64   # dry run
...
@torch.no_grad()
def extract_split(split, model, processor, limit=None):
    ...
    out = Path(P["features_dir"]) / f"{split}.h5"
...
def verify(split):
    ...
    print(f"[{split}] complete={ok}  written={nw:,}/{tot:,}  "
...
def main():
    ...
    processor = AutoImageProcessor.from_pretrained(CFG["models"]["vision_encoder"])
